...
const app = express();
//const app2 = express();
const directoryToServe = path.join(__dirname);  // Change this to your desired folder
...
app.use((req, res, next) => {
  const forbiddenPaths = ['/server.js', '/python/https-server.py', '/mainlinejcdbme.js', '/doc.js'];  // List of forbidden paths
...
app.use(express.static(directoryToServe));  // Assuming your static files are in the "public" directory
...
<body>
    <div class="container">
        ...
        <a href="https://192.168.254.161:8080">Go to Homepage</a>
    </div>
</body>
</html>
  `);
...
app.use(express.static(path.join(__dirname, '/images/favicon.ico')));

// Create an HTTPS server
const options = {
  key: fs.readFileSync('/Users/anirudhsevugan/jcdbmeservercerts/server.key'),
  cert: fs.readFileSync('/Users/anirudhsevugan/jcdbmeservercerts/server.crt'),
};
...
httpsServer.listen(httpsPort, () => {
  console.log(`HTTPS main server running on https://192.168.254.161:${httpsPort}`);
  console.log(`HTTP backup server started on http://192.168.254.161:${httpsBackupPort}`);
});

const targetDirectory = path.join("/Users/anirudhsevugan/JCDBME"); // Adjust this path
const targetDirectory2 = path.join(__dirname, "/misc/websocket-chat"); // Adjust this path


// Command to run the Python HTTP server
// Run npx http-server with sudo
const pythonProcess = spawn('npx', [
  'http-server',
  '-p',
  '4443',
], {
...
const child = spawn('npx', ['http-server', '-p', '8000'], {
...
// Spawn the child process
const child3 = spawn('node', ['doc.js'], {
});
